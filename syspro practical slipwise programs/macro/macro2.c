4.A.2>Write a program for macro preprocessor which will Create a file trial.asm as follows
trial.asm
MARCO
CALC &X,&Y,&REG=BREG,&OP=ADD
MOVER &REG,&X
&OP &REG,&Y
MOVEM &REG,&X
MEND
...
void Make_KPDT_PNT(char *s)
{
	...
	while(*s && *s!='=')
	{
		if(*s==',')
		{
			...
			{
				printf("\nError: Multiple Declaration of Symbol %s in Argument 
...
void main()
{
	...
	fclose(fp);}
******************OUTPUT:*********************************
Enter Source File Name: trial.txt
